...
```{r}
person_data <- read_xml("https://raw.githubusercontent.com/krpopkin/DATA607-Class-Repository/master/Week_7/xml/person.xml")
```

```{r}
books_data <- read_xml("https://raw.githubusercontent.com/krpopkin/DATA607-Class-Repository/master/Week_7/xml/books_for_assignment5.xml")
```
...
```{r}
result <- xmlParse(books_data)
```

```{r}
xmldf <- xmlToDataFrame(result)
```
